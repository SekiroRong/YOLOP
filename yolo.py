...
class YOLO(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'G:\yoloR\logs/ep002-loss0.804-val_loss3.786.pth',
    ...
    def detect_image(self, image, Dimage = None):
        ...
        with torch.no_grad():
            ...
            # ll_predict += 50
            # print(da_predict)
            # da_predict[da_predict < 0] = 0
            # ll_predict[ll_predict < 0] = 0
            # print(da_predict)
            # da_seg_mask = torch.nn.functional.interpolate(da_predict, scale_factor=int(2), mode='bilinear')
            # _, da_seg_mask = torch.max(da_seg_mask, 1)
            # # print(da_seg_mask.shape)
            # da= da_seg_mask.int().squeeze().cpu()
            # da = np.array(da,dtype=np.uint8)
            ll_predict = np.array(ll_predict)
            da_predict = np.array(da_predict)

            # ll_min, ll_max, ll_mean = np.min(ll_predict), np.max(ll_predict), np.mean(ll_predict)
            # da_min, da_max, da_mean = np.min(da_predict), np.max(da_predict), np.mean(da_predict)
            ll_min = np.min(ll_predict)
            da_min = np.min(da_predict)
            ll_predict -= ll_min
            da_predict -= da_min
            # ll_predict /= ((ll_max-ll_min)/255)
            ll_predict[ll_predict > 255] = 255
            da_predict[da_predict > 255] = 255
            # print(ll_min, ll_max,ll_mean)
            # print(np.min(ll_predict), np.max(ll_predict))
